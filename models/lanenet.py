...
class LaneNet(nn.Module):
    ...
    def net_init(self):
        ...
        self.binary_seg = nn.Sequential(
            nn.Conv2d(8, 8, 1), nn.BatchNorm2d(8), nn.ReLU(), nn.Conv2d(8, 2, 1)
        )
    ...
    def forward(self, img, segLabel=None):
        ...
        loss = (
            seg_loss * self.scale_seg
            + var_loss * self.scale_var
            + dist_loss * self.scale_dist
        )
        ...
        return binary_seg, embedding, loss
            ...
            for lane_idx in labels:
                ...
                var_loss = (
                    ...
                    + torch.mean(
                        F.relu(
                            torch.norm(
                                embedding_i - mean_i.reshape(self.embed_dim, 1), dim=0
                            )
                            - self.delta_v
                        )
                        ** 2
    ...
    def fit(self, num_epochs, train_dataloader, optimizer, lr_scheduler, device):
        losses_epoch = []
        train_accuracies = []
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for img, binary_target, *_ in tqdm(train_dataloader):
                ...
                binary_logits, instance_emb, loss = self(img, binary_target)
            ...
            accuracy = correct_binary / total_pixels
            train_accuracies.append(accuracy)

            mean_losses = np.array(losses).mean()
            losses_epoch.append(mean_losses)
        ...
        return losses_epoch, train_accuracies
    ...
    def predict(self, image):
        ...
        with torch.no_grad():
            binary_logits, instance_logits, _ = self(image.repeat(3, 1, 1).unsqueeze(0))
