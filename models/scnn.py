...
class SCNN(nn.Module):
    def __init__(self, input_size=(512, 256), ms_ks=9, pretrained=True):
        """
        Argument
    ...
    def net_init(self, input_size, ms_ks):
        ...
        self.layer2 = nn.Sequential(
            nn.Dropout2d(0.1), nn.Conv2d(128, 7, 1)  # get (nB, 5, 36, 100)
        )
    ...
    def fit(self, num_epochs, train_dataloader, optimizer, lr_scheduler, device):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for img, binary_target, *_ in tqdm(train_dataloader):
                ...
                loss = self.ce_loss(seg_pred, binary_target.long())
                ...
                binary_preds = torch.argmax(seg_pred, dim=1)
    ...
    def predict(self, image):
        ...
        with torch.no_grad():
            seg_pred, *_ = self(image.repeat(3, 1, 1).unsqueeze(0))

        binary_seg = torch.argmax(seg_pred, dim=1).squeeze().numpy()
