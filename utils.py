...
def accuracy_score(image, image_pred):
    diferencia = cv2.absdiff(image, image_pred)

    return 1 - cv2.mean(diferencia)[0] / (
        255 if np.max(image) > 1 or np.max(image_pred) > 1 else 1
    )
